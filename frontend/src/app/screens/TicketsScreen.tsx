...
import { getMyTickets, type ApiTicket } from "../../api/tickets";
...
function QrModal({ ticket, onClose }: QrModalProps) {
  ...
  const qrValue = JSON.stringify({ ticketId: ticket.id, eventId: ticket.event_id });
    ...
    >
      ...
      >
        ...
        <div className="text-center">
          <p className="font-medium">{ticket.event_name}</p>
          <p className="text-sm text-muted-foreground">
            {new Date(ticket.event_date).toLocaleDateString("de-DE")}
          </p>
...
export function TicketsScreen() {
  ...
  const [qrTicket, setQrTicket] = useState<ApiTicket | null>(null);
  ...
  const now = Date.now();
  const activeTickets = tickets.filter((t) => new Date(t.event_date).getTime() >= now);
  const pastTickets = tickets.filter((t) => new Date(t.event_date).getTime() < now);
              ...
              >
                <div className="flex gap-3 mb-3">
                  <div
                    ...
                    style={{
                      backgroundImage: getImageUrl(ticket.event_image_path)
                        ? `url(${getImageUrl(ticket.event_image_path)})`
                        : "linear-gradient(135deg, #1D9E75 0%, #D85A30 100%)",
                    }}
                  />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <h3 className="truncate">{ticket.event_name}</h3>
              ...
              <Card key={ticket.id} className="p-4 opacity-60">
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <h3 className="truncate">{ticket.event_name}</h3>
                      <Badge className="bg-gray-500 text-white hover:bg-gray-500">Vergangen</Badge>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      {new Date(ticket.event_date).toLocaleDateString("de-DE")}
                    </p>
